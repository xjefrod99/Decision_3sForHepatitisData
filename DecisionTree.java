...
public class DecisionTree {
	...
	private void train(TreeNode node, int numFeatures) {
		...
		// 7. If no more features (Base Case 3)
			...
			if(best_split == -1 && node.parent != null){
				//in the case that we dont find a feature with inf gain, then we set the node's decision based on parents' majority
				TreeNode par = node.parent; //assuming there's always a parent node
				int par_pos = par.pos.size();
				int par_neg = par.neg.size();
				if (par_pos >= par_neg) 
	...
	private int calculate_best(TreeNode node, int numFeatures){
		...
		for(int i =0; i < numFeatures; i++){ //<numFeatures
			...
			if(node.featureUsed(i) == false){ 
				...
				if (  getRemainingEntropy( i, node) < inf_gain){
					best_i = i;
					inf_gain = getRemainingEntropy( i, node);
	...
	private void createChildren(TreeNode node, int numFeatures) {
		...
		for( Example e: node.pos){
			if ( e.getFeatureValue(feature) && e.getLabel() )
				truePos.add(e);
			else
				trueNeg.add(e);
		}
		...
		for( Example e: node.neg){
			if (  e.getFeatureValue(feature) && e.getLabel() == false)
				falsePos.add(e);
			else
				falseNeg.add(e);
		}
		//creating trueChild and false Child
	...
	private double getRemainingEntropy(int feature, TreeNode node) {
		...
		double weight = (posA + posB)/total;
		double entropy = weight * getEntropy(posA, negA) + ((negA+negB)/total) *getEntropy(posB, negB);
		return entropy;
	}
	...
	private double getEntropy(int numPos, int numNeg) {
		...
		// if (numNeg == numPos)
		// 	return 1.0;
		if (numPos == 0 || numNeg == 0)//there is no entropy
			return 0.0;
